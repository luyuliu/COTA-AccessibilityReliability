...
class DijkstraSolver(BasicSolver.BasicSolver):
    def __init__(self, args):
        ...
        # The start timestamp when the hypothesis user leaves the start point, e.g. 8am, 12pm, 6pm
        timestamp = args[0]
        # Walking distance limit, usually 700 meters
        walkingDistanceLimit = args[1]
        # A hard limit of stop time limit. The buses after that time limit will not be considered as a possible trip, even if it exists. Uusally 3 hours = 180 * 60 seconds
        ...
        # The limit of scooter travel disctance. Not used in this project.
        ...
        # Limit the time to just one day.
        self.timeLimit = min(self.todaySeconds + 86400,
                             timestamp + timeDeltaLimit)
        ...
        self.rl_stop_times_rt = list(self.col_real_times.find(
            {"time": {"$gt": timestamp, "$lt": self.timeLimit}}))  # Real-time
        self.rl_stop_times_sc = list(self.col_real_times.find(
            {"scheduled_time": {"$gt": timestamp, "$lt": self.timeLimit}}))  # Scheduled
        ...
        for startStopID, startStop in self.arcsDicRT.items():
            for endStopID, endStop in startStop.items():
                self.arcsDicRT[startStopID][endStopID] = OrderedDict(
                    sorted(endStop.items(), key=return_time_gen))

        for startStopID, startStop in self.arcsDicSC.items():
            for endStopID, endStop in startStop.items():
                self.arcsDicSC[startStopID][endStopID] = OrderedDict(
                    sorted(endStop.items(), key=return_time_gen))
    ...
    # Get the travel time (network cost) between the two stops given the arrival time at the start stop
    def getTravelTimeRT(self, closestStopID, eachStopID, aStartTime):
        ...
        try:
            ...
            for eachIndex, eachTrip in thisArcsDic.items():
                ...
                #     a = 1
                # This requires that arcDic[Start][End] to be an ascending sequence. So use OrderedDic to maintain this feature.
                if eachTrip["time_gen"] >= aStartTime:
                    ...
                    travelTime["timeRT"] = travelTime["busTimeRT"] + \
                        travelTime["waitTimeRT"]
        ...
        # cannot make two subsequent non-transit arcs.
        if self.visitedSet[closestStopID]["lastTripTypeRT"] == "walk":
            return travelTime

        dist = self.calculateDistance(
            self.stopsDic[closestStopID], self.stopsDic[eachStopID])  # Can be precalculated
        walkTime_walk = dist/self.walkingSpeed
        totalTime_walk = walkTime_walk

        if totalTime_walk > self.walkingTimeLimit:
            pass
        # if no bus trip, travelTime["time"] should be maxsize; or, the bus trip is very slow, but very unlikely
        elif travelTime["timeRT"] > totalTime_walk:
    ...
    def getTravelTimeSC(self, closestStopID, eachStopID, aStartTime):
        ...
        try:
            ...
            for eachIndex, eachTrip in self.arcsDicSC[closestStopID][eachStopID].items():
                if eachTrip["time_gen"] >= aStartTime:
                    ...
                    travelTime["timeSC"] = travelTime["busTimeSC"] + \
                        travelTime["waitTimeSC"]
        ...
        # No edge, can only walk/scooter
        # cannot make two subsequent non-transit arcs.
        if self.visitedSet[closestStopID]["lastTripTypeSC"] == "walk":
            return travelTime
        dist = self.calculateDistance(
            self.stopsDic[closestStopID], self.stopsDic[eachStopID])  # Can be precalculated
        walkTime_walk = dist/self.walkingSpeed
        ...
        if totalTime_walk > self.walkingTimeLimit:
            pass
        # if no bus trip, travelTime["time"] should be maxsize; or, the bus trip is very slow, but very unlikely
        elif travelTime["timeSC"] > totalTime_walk:
    ...
    def addToTravelTime(self, originalStrucuture, closestStructure, travelTime, tempStartTimestamp, isRT=True):
        if isRT:
            originalStrucuture["timeRT"] = closestStructure["timeRT"] + \
                travelTime["timeRT"]
            originalStrucuture["walkTimeRT"] = closestStructure["walkTimeRT"] + \
                travelTime["walkTimeRT"]
            originalStrucuture["busTimeRT"] = closestStructure["busTimeRT"] + \
                travelTime["busTimeRT"]
            originalStrucuture["waitTimeRT"] = closestStructure["waitTimeRT"] + \
                travelTime["waitTimeRT"]
            ...
            originalStrucuture["timeSC"] = closestStructure["timeSC"] + \
                travelTime["timeSC"]
            originalStrucuture["walkTimeSC"] = closestStructure["walkTimeSC"] + \
                travelTime["walkTimeSC"]
            originalStrucuture["busTimeSC"] = closestStructure["busTimeSC"] + \
                travelTime["busTimeSC"]
            originalStrucuture["waitTimeSC"] = closestStructure["waitTimeSC"] + \
                travelTime["waitTimeSC"]
    ...
    def extendStops(self, startStopID):  # Corresponded to dijjkstra.docx, Line 1
        ...
        for _ in (self.rl_stops):  # Line 8. Instead of using a while loop, I use a for loop to enumerate all the stops, which is equivilent except some redundants
            # Line 9: find the closest node in set Q to the S set, which is the ones having the confirmed clostest distance
            closestStop = self.findClosestStop(True)
            if closestStop == False:  # Cannot find any closest nodes, every node is visited.
                # print("break!")
                break
            closestStopID = closestStop["stop_id"]

            # Line 10: remove u from Q
            self.visitedSet[closestStopID]["visitTagRT"] = True

            # Can modify self.rl_stops to improve performance.

            # Line 11: For every neighbor of the closest stop to the set S, which is the ones having the confirmed closest distance.
            for eachStop in (self.rl_stops):
                eachStopID = eachStop["stop_id"]
                # Only for vertices that not in S.
                if self.visitedSet[eachStopID]["visitTagRT"] == True:
                    continue

                tempStartTimestamp = startTimestamp + \
                    self.visitedSet[closestStopID]["timeRT"]  # Line 12
                # Find the weight between closestStop and eachStop
                travelTime = self.getTravelTimeRT(
                    closestStopID, eachStopID, tempStartTimestamp)
                ...
                if self.visitedSet[eachStopID]["timeRT"] > self.visitedSet[closestStopID]["timeRT"] + travelTime["timeRT"]:  # Line 13
                    self.visitedSet[eachStopID] = self.addToTravelTime(
                        self.visitedSet[eachStopID], self.visitedSet[closestStopID], travelTime, tempStartTimestamp, True)  # Line 14 -15
        ...
        for _ in (self.rl_stops):
            # Dijkstra's algorithm: find the closest node to the S set, which is the ones having the confirmed clostest distance.
            closestStop = self.findClosestStop(False)
            if closestStop == False:
                # print("break!")
                break
            closestStopID = closestStop["stop_id"]

            # Add closest stop to S set.
            ...
            for eachStop in (self.rl_stops):
                ...
                if self.visitedSet[eachStopID]["visitTagSC"] == True:
                    continue
                tempStartTimestamp = startTimestamp + \
                    self.visitedSet[closestStopID]["timeSC"]
                travelTime = self.getTravelTimeSC(
                    closestStopID, eachStopID, tempStartTimestamp)
                # if travelTime["tripType"] != None:
                #     print(self.visitedSet[eachStopID]["time"] > self.visitedSet[closestStopID]["time"] + travelTime["time"])
                ...
                if travelTime['timeSC'] > 0 and self.visitedSet[eachStopID]["visitTagSC"] == False and self.visitedSet[eachStopID]["timeSC"] > self.visitedSet[closestStopID]["timeSC"] + travelTime["timeSC"]:
                    self.visitedSet[eachStopID] = self.addToTravelTime(
                        self.visitedSet[eachStopID], self.visitedSet[closestStopID], travelTime, tempStartTimestamp, False)
...
# Calculate travel time from startLocation to all other stops.
...
def collectiveAccessibilitySolve(args, sampledStopsList):
    ...
    for i in tqdm(list(range(batch))):
        ...
        sub_output = pool.starmap(singleAccessibilitySolve, zip(
            [args]*len(sub_sampledStopsList), sub_sampledStopsList))
        pool.close()
        pool.join()
    ...
    for singleDate in (daterange):
        ...
        for eachTimestamp in todayTimestampList:
            ...
            args = [int(eachTimestamp), walkingDistanceLimit, timeDeltaLimit,
                    walkingSpeed, scooterSpeed, scooterDistanceLimit, isRealTime, isScooter]

            resultsFeedback = collectiveAccessibilitySolve(
                args, sampledStopsList)
